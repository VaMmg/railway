<?php
// BACKEND/api/perfiles_crediticio.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM perfiles_crediticio WHERE id_perfil = :id";
        ...
        $sql = "SELECT * FROM perfiles_crediticio ORDER BY id_perfil DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM perfiles_crediticio";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO perfiles_crediticio (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_perfil' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE perfiles_crediticio SET " . implode(', ', $updateFields) . " WHERE id_perfil = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM perfiles_crediticio WHERE id_perfil = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
