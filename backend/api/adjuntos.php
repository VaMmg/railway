<?php
// BACKEND/api/adjuntos.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM adjuntos WHERE id_adjunto = :id";
        ...
        $sql = "SELECT * FROM adjuntos ORDER BY id_adjunto DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM adjuntos";
...
function handlePost() {
    ...
    $required = ['entidad_origen', 'id_entidad', 'nombre_archivo'];
    ...
    try {
        ...
        $sql = "INSERT INTO adjuntos (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_adjunto' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE adjuntos SET " . implode(', ', $updateFields) . " WHERE id_adjunto = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
