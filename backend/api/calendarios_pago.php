<?php
// BACKEND/api/calendarios_pago.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM calendarios_pago WHERE id_calendario_pago = :id";
        ...
        $sql = "SELECT * FROM calendarios_pago ORDER BY id_calendario_pago DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM calendarios_pago";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO calendarios_pago (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_calendario_pago' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE calendarios_pago SET " . implode(', ', $updateFields) . " WHERE id_calendario_pago = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM calendarios_pago WHERE id_calendario_pago = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
