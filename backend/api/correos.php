<?php
// BACKEND/api/correos.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM correos WHERE id_correo = :id";
        ...
        $sql = "SELECT * FROM correos ORDER BY id_correo DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM correos";
...
function handlePost() {
    ...
    $required = ['dni_persona', 'correo_principal'];
    ...
    try {
        ...
        $sql = "INSERT INTO correos (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_correo' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE correos SET " . implode(', ', $updateFields) . " WHERE id_correo = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM correos WHERE id_correo = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
