<?php
// BACKEND/api/fiadores.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM fiadores WHERE id_fiador = :id";
        ...
        $sql = "SELECT * FROM fiadores ORDER BY id_fiador DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM fiadores";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO fiadores (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_fiador' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE fiadores SET " . implode(', ', $updateFields) . " WHERE id_fiador = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
