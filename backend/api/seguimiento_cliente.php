<?php
// BACKEND/api/seguimiento_cliente.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM seguimiento_cliente WHERE id_seguimiento = :id";
        ...
        $sql = "SELECT * FROM seguimiento_cliente ORDER BY id_seguimiento DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM seguimiento_cliente";
...
function handlePost() {
    ...
    $required = ['id_cliente', 'tipo_contacto'];
    ...
    try {
        ...
        $sql = "INSERT INTO seguimiento_cliente (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_seguimiento' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE seguimiento_cliente SET " . implode(', ', $updateFields) . " WHERE id_seguimiento = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM seguimiento_cliente WHERE id_seguimiento = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
