<?php
// BACKEND/api/garantias.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM garantias WHERE id_garantia = :id";
        ...
        $sql = "SELECT * FROM garantias ORDER BY id_garantia DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM garantias";
...
function handlePost() {
    ...
    $required = ['id_credito', 'tipo_garantia'];
    ...
    try {
        ...
        $sql = "INSERT INTO garantias (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_garantia' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE garantias SET " . implode(', ', $updateFields) . " WHERE id_garantia = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM garantias WHERE id_garantia = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
