<?php
// BACKEND/api/cliente_trabajo.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM cliente_trabajo WHERE id_trabajo = :id";
        ...
        $sql = "SELECT * FROM cliente_trabajo ORDER BY id_trabajo DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM cliente_trabajo";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO cliente_trabajo (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_trabajo' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE cliente_trabajo SET " . implode(', ', $updateFields) . " WHERE id_trabajo = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM cliente_trabajo WHERE id_trabajo = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
