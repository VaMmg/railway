<?php
// BACKEND/api/plantillas_contrato.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM plantillas_contrato WHERE id_plantilla = :id";
        ...
        $sql = "SELECT * FROM plantillas_contrato ORDER BY id_plantilla DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM plantillas_contrato";
...
function handlePost() {
    ...
    $required = ['nombre_plantilla', 'contenido_html'];
    ...
    try {
        ...
        $sql = "INSERT INTO plantillas_contrato (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_plantilla' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE plantillas_contrato SET " . implode(', ', $updateFields) . " WHERE id_plantilla = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM plantillas_contrato WHERE id_plantilla = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
