<?php
// BACKEND/api/seguros_credito.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM seguros_credito WHERE id_seguro_credito = :id";
        ...
        $sql = "SELECT * FROM seguros_credito ORDER BY id_seguro_credito DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM seguros_credito";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO seguros_credito (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_seguro_credito' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE seguros_credito SET " . implode(', ', $updateFields) . " WHERE id_seguro_credito = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM seguros_credito WHERE id_seguro_credito = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
