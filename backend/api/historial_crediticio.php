<?php
// BACKEND/api/historial_crediticio.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM historial_crediticio WHERE id_historial = :id";
        ...
        $sql = "SELECT * FROM historial_crediticio ORDER BY id_historial DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM historial_crediticio";
...
function handlePost() {
    ...
    $required = ['id_credito', 'tipo_evento'];
    ...
    try {
        ...
        $sql = "INSERT INTO historial_crediticio (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_historial' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE historial_crediticio SET " . implode(', ', $updateFields) . " WHERE id_historial = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
