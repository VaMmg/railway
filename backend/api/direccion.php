<?php
// BACKEND/api/direccion.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM direccion ORDER BY id_direccion DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM direccion";
...
function handlePost() {
    ...
    $required = ['dni_persona', 'direccion_principal'];
    ...
    try {
        ...
        $sql = "INSERT INTO direccion (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_direccion' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE direccion SET " . implode(', ', $updateFields) . " WHERE id_direccion = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
