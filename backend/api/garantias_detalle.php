<?php
// BACKEND/api/garantias_detalle.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM garantias_detalle WHERE id_garantia_detalle = :id";
        ...
        $sql = "SELECT * FROM garantias_detalle ORDER BY id_garantia_detalle DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM garantias_detalle";
...
function handlePost() {
    ...
    $required = ['id_garantia', 'tipo_detalle'];
    ...
    try {
        ...
        $sql = "INSERT INTO garantias_detalle (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_garantia_detalle' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE garantias_detalle SET " . implode(', ', $updateFields) . " WHERE id_garantia_detalle = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM garantias_detalle WHERE id_garantia_detalle = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
