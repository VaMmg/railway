<?php
// BACKEND/api/contacto.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM contacto ORDER BY id_contacto DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM contacto";
...
function handlePost() {
    ...
    $required = ['dni_persona', 'numero_principal'];
    ...
    try {
        ...
        $sql = "INSERT INTO contacto (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_contacto' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE contacto SET " . implode(', ', $updateFields) . " WHERE id_contacto = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
