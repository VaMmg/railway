<?php
// BACKEND/api/comisiones_credito.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM comisiones_credito WHERE id_comision = :id";
        ...
        $sql = "SELECT * FROM comisiones_credito ORDER BY id_comision DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM comisiones_credito";
...
function handlePost() {
    ...
    $required = ['id_credito', 'tipo_comision', 'monto'];
    ...
    try {
        ...
        $sql = "INSERT INTO comisiones_credito (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_comision' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE comisiones_credito SET " . implode(', ', $updateFields) . " WHERE id_comision = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM comisiones_credito WHERE id_comision = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
