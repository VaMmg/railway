<?php
// BACKEND/api/comentarios.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM comentarios ORDER BY id_comentario DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM comentarios";
...
function handlePost() {
    ...
    $required = ['id_credito', 'comentario'];
    ...
    try {
        ...
        $sql = "INSERT INTO comentarios (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_comentario' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE comentarios SET " . implode(', ', $updateFields) . " WHERE id_comentario = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
