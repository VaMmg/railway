<?php
// BACKEND/api/credito_etiqueta.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM credito_etiqueta WHERE id_credito_etiqueta = :id";
        ...
        $sql = "SELECT * FROM credito_etiqueta ORDER BY id_credito_etiqueta DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM credito_etiqueta";
...
function handlePost() {
    ...
    $required = ['id_credito', 'id_etiqueta'];
    ...
    try {
        ...
        $sql = "INSERT INTO credito_etiqueta (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_credito_etiqueta' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE credito_etiqueta SET " . implode(', ', $updateFields) . " WHERE id_credito_etiqueta = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM credito_etiqueta WHERE id_credito_etiqueta = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
