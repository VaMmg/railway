<?php
// BACKEND/api/sucursal.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM sucursal ORDER BY id_sucursal DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM sucursal";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO sucursal (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_sucursal' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE sucursal SET " . implode(', ', $updateFields) . " WHERE id_sucursal = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
