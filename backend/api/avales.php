<?php
// BACKEND/api/avales.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM avales WHERE id_aval = :id";
        ...
        $sql = "SELECT * FROM avales ORDER BY id_aval DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM avales";
...
function handlePost() {
    ...
    $required = ['id_credito', 'id_persona'];
    ...
    try {
        ...
        $sql = "INSERT INTO avales (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_aval' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE avales SET " . implode(', ', $updateFields) . " WHERE id_aval = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM avales WHERE id_aval = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
