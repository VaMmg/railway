<?php
// BACKEND/api/etiquetas_credito.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM etiquetas_credito WHERE id_etiqueta = :id";
        ...
        $sql = "SELECT * FROM etiquetas_credito ORDER BY id_etiqueta DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM etiquetas_credito";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO etiquetas_credito (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_etiqueta' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE etiquetas_credito SET " . implode(', ', $updateFields) . " WHERE id_etiqueta = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM etiquetas_credito WHERE id_etiqueta = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
