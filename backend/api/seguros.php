<?php
// BACKEND/api/seguros.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM seguros ORDER BY id_seguro DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM seguros";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO seguros (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_seguro' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE seguros SET " . implode(', ', $updateFields) . " WHERE id_seguro = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
