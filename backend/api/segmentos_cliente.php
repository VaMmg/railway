<?php
// BACKEND/api/segmentos_cliente.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM segmentos_cliente WHERE id_segmento = :id";
        ...
        $sql = "SELECT * FROM segmentos_cliente ORDER BY id_segmento DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM segmentos_cliente";
...
function handlePost() {
    ...
    try {
        ...
        $sql = "INSERT INTO segmentos_cliente (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_segmento' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE segmentos_cliente SET " . implode(', ', $updateFields) . " WHERE id_segmento = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM segmentos_cliente WHERE id_segmento = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
