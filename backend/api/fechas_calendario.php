<?php
// BACKEND/api/fechas_calendario.php - API Auto-generada
...
function handleGet() {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT * FROM fechas_calendario WHERE id_fecha = :id";
        ...
        $sql = "SELECT * FROM fechas_calendario ORDER BY id_fecha DESC LIMIT :limit OFFSET :offset";
        $stmt = $pdo->prepare($sql);
        ...
        $countSql = "SELECT COUNT(*) as total FROM fechas_calendario";
...
function handlePost() {
    ...
    $required = ['id_calendario_pago', 'fecha'];
    ...
    try {
        ...
        $sql = "INSERT INTO fechas_calendario (" . implode(', ', $fields) . ") VALUES ($placeholders)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($input);
        
        $id = $pdo->lastInsertId();
        jsonResponse(true, 'Registro creado exitosamente', ['id_fecha' => $id], 201);
...
function handlePut() {
    ...
    try {
        ...
        if (!empty($updateFields)) {
            $sql = "UPDATE fechas_calendario SET " . implode(', ', $updateFields) . " WHERE id_fecha = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        }
...
function handleDelete() {
    ...
    try {
        $sql = "DELETE FROM fechas_calendario WHERE id_fecha = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':id' => $id]);
